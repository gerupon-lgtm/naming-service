...
import { generateComment, type DiagnosisPayload } from "./_lib/comment";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (body.type !== "diagnosis" || !body.payload) {
    return res
      .status(400)
      .json({ error: "INVALID_INPUT", message: "type と payload が必要です" });
  }

  try {
    const comment = await generateComment("diagnosis", body.payload);
