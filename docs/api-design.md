...
- **よみ逆引き（F-004・実装済）**: `reading` 指定時、まず かな候補（もなか／モナカ）を先頭に出す。さらに漢字が許可されていれば、よみを**モーラ分割→各セグメントを kanjiapi.dev `GET /v1/reading/{seg}` で漢字化→合成**する。名前らしさのため **(1)漢字名は1〜2文字まで (2)主要な読み(main_kanji)優先 (3)「名前向きの常用漢字」許可リスト（`db/seed/name_kanji_allow.json`・新聞頻度＋教育漢字 約2,500字）に限定** して稀字・不自然な字を排除。よみ由来は最大6件（努力目標3件以上）、全体は最大 `SUGGEST_DEFAULT_COUNT`（8）。kanjiapi.dev 障害時は漢字候補をスキップ。
