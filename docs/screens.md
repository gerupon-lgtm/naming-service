...
- **五行ボーナスブロック（F-015・実装済）**: 生年月日の入力があった場合のみ、**総合ランクの枠外に独立ブロック**として表示（スコアに影響しないことが見た目で分かる配置）。
  ...
  - **導入文＋取り入れ方（v2.4.2）**: 見出し直下に①導入文（「相性の目安・後押し・点数に影響しない」）を常時表示。本文の下に②折りたたみ「吉となる五行の取り入れ方をみる」を置き、**用神に該当する五行だけ**、色・方角・食べ物・小物の提案を表示（あくまで提案・開運の楽しみ）。下部注釈は短縮版（ツールチップは維持）。導入文・注釈は保存/コピー出力にも反映（ツールチップ無し版）
