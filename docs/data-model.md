...
## 四柱推命連携（F-015）の永続化・プライバシー方針

- **適用範囲**: 突合は**姓名診断モード（S-001/S-002）のみ**。ペット命名モードでは生年月日を受け取らない（ペットの生年月日は不明なことが多いため）。
- **生年月日・出生時刻はDBに保存しない**（診断結果と同じく非永続）。リクエスト処理中のみメモリ保持し、応答後は破棄する。ログ・URL・LLMの生プロンプトにも生年月日を載せない（LLMへ渡すのは不足五行など抽象化済み情報のみ）。
- **共有URL（F-006）は従来どおり `sei`/`mei`/`sex` のみ**。生年月日をクエリに含めないため、共有URLで開いた結果に五行突合は付かない（仕様）。
- **五行突合は総合スコア・ランク・五格を変えない**（「四柱推命による五行ボーナス」= `wuxingBonus` として別枠表示）。共有URLの再現性と「同じ名前・同じ性別なら同じ結果」という姓名判断の一貫性を守るための制約。評価は星4段階（★★★／★★☆／★☆☆／☆☆☆=恩恵なし）で、0〜100の数値は持たない。度合いのラベルは画面に出さず、星と本文で伝える。
- 連携で用いる五行は都度計算。`name_master`・`character_master` に**スキーマ変更は不要**。
- 名前の五行は「画数の一の位」方式（`wuxingOf`）、人の五行は四柱推命方式（`api/_lib/bazi/`・Phase A で内蔵予定）で求める**役割分担**を厳守する。詳細・データ互換性・決定事項は `docs/integration-shichu.md` を正とする。
