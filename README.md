...
### Phase1（MVP）は完了・本番公開済み
- 本番URL: https://naming-service-red.vercel.app
- 残る発展余地: 本格シードの拡充、独自ドメイン、四柱推命アプリとの五行連携、Phase2（ペット命名提案 F-002〜）。
...
- **五行サマリ（`wuxing`）**: 三才の五行内訳と不足五行を構造化出力。**四柱推命など五行を使う
  他占術との連携用**（他アプリが `wuxing.lacking` 等を取り込む想定。連携配線は相手側の
  データ受け渡し方法が決まり次第）。
