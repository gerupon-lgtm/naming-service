...
| T-320 | **v2.2.0**: ボーナス用語のツールチップ化、日付・時刻アイコンのSVG化（スマホで非表示になる不具合の修正） | ✅ 実装 |
| T-321 | **v2.3.0**: 共有URLに用神リスト `wx` を付与（生年月日は載せない）。共有先でも五行ボーナスを再現 | ✅ 実装 |
| T-322 | **v2.3.0**: 使いたい文字をLLM生成に切替。かなは「よみ」、漢字は「表記」で照合。指定文字がよみに含まれるかを機械検証 | ✅ 実装 |
| T-323 | **v2.3.1**: 候補の由来を区別（`source: chars`）し、チップの誤表示を修正 | ✅ 実装 |
| T-330〜335 | **v2.4.0**: 使いたい文字を単体1文字・表記照合に再定義。ローマ字出力（ヘボン式）＋アルファベット対応。使いたい文字×よみの2段構え＋お知らせ。設計 `docs/pet-usable-char.md` | ✅ 実装 |
| 単体テスト（追加後） | 上記＋既存すべて（**約210件**／21ファイル・重複除く） | ✅ 全パス |
...
- **Phase2（ペット命名提案）**: 条件（対象・性別・雰囲気・使いたい文字・希望よみ・出力文字種）で候補を提示。**ユーザー希望由来はLLM生成、不足分はマスタからランダム**。
  - 希望よみの**漢字表記はLLM生成**（自然な表記を6個生成→先頭2＋画数上位を採用）
  - **使いたい文字は単体1文字を「表記」に含める**（v2.4.0で再定義。かな・漢字・カタカナ・アルファベット）。アルファベットは**ローマ字表記**（ヘボン式）の名前になる。マスタでは組合せを賄えないためLLM生成
  - **使いたい文字×希望のよみが両立しないときはよみを優先**し、お知らせを表示（2段構え）
  - **LLM出力は必ず機械検証する**（指定文字が本当によみに含まれるか、画数が引けるか）。LLMは条件を守らないことがあるため、生成させっぱなしにしない
  - LLM応答不可時はマスタのみで動作（マスタはフォールバックとして維持）
  - 候補ごとのLLMコメント（よみ中心・日本語のみ・1件ずつ順次・同じよみは統一）
