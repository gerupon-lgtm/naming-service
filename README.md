...
| 単体テスト（追加後） | 上記＋既存すべて（**156件**／18ファイル） | ✅ 全パス |
...
- **Phase3（四柱推命連携 F-015）**: 生年月日（任意）から**用神**（吉となる五行）を求め、「四柱推命による五行ボーナス」として**別枠**で参考表示。3段階の言葉＋星（★★★／★★☆／★☆☆）で、**総合ランク・スコア・五格は一切変えない**。出生時刻・出生地も任意（L1三柱→L2四柱→L3時差補正）。実装は `api/_lib/bazi/`、設計は `docs/integration-shichu.md`。
